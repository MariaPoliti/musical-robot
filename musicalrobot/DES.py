...
# Step: Generate a large amount of possibilities, narrow down using constraints
# Input: min and max mol fraction desired, desired output samples,
#  with trial estimate
...
def des_generator(min_comps, max_comps, samples, trials):
    ...
    # Generating random DES compositions within the design contraints,
    # mole fractions of each composition must = 1.
    ...
    # This normalization may still lead to compositions that do not satisfy
    # the constraint
    ...
    print(" "+str(len(Feasible_DES_samples)) +
          " feasible DES samples generated, clustered into " +
          str(samples) + " samples")

    # Apply K-means clustering to DES samples
    kmeans = KMeans(n_clusters=samples, random_state=0).fit(
        Feasible_DES_samples)
    DES_molfrac = kmeans.cluster_centers_
    return DES_molfrac


# Function: Converts mol fractions to volumes for pipetting and reference
# Step: create system of equations, solve it to determine the volume of each
...
def open_vol(final_vol):
    '''
    Converts the array of volumes with comp volumes to two lists
    of separate volumes
